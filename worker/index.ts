...
const demoListings: Listing[] = [
  { id: "thegraph/substreams-deployer", namespace: "thegraph", slug: "substreams-deployer", title: "Substreams Deployer", summary: "Turn a protocol prompt into a deployable Substreams pipeline.", category: "The Graph", priceUsdc: "0.25", paidInstalls: 1842, trend: 28, author: "thegraph", authorAddress: "0x8dF2000000000000000000000000000000007A31", version: "1.4.0", updatedAt: "Local demo" },
  { id: "defi/audited-automation", namespace: "defi", slug: "audited-automation", title: "Audited DeFi Automation", summary: "Guardrails for recurring DeFi workflows.", category: "DeFi", priceUsdc: "0.80", paidInstalls: 912, trend: 16, author: "defi", authorAddress: "0xA92E0000000000000000000000000000019B8", version: "1.2.1", updatedAt: "Local demo" },
]
...
async function graphListings(env: Bindings) {
  ...
  return (payload.data?.skills ?? []).map((skill, index): Listing => {
    ...
    return { ...fallback, id: parsed ? sourceId : skill.id, namespace, slug, title: slug.split("-").map((word) => word[0].toUpperCase() + word.slice(1)).join(" "), summary: "A paid agent skill published on SkillsBay.", category: "Agent skill", priceUsdc: (Number(skill.price) / 1_000_000).toFixed(2), paidInstalls: skill.totalSales, trend: 0, author: namespace, authorAddress: skill.author.id, version: `${skill.majorVersion}.0.0`, updatedAt: "Indexed on Base Sepolia" }
  })
}

async function listings(env: Bindings) {
  const allowDemoData = env.APP_ENV !== "production"
  const fallback = allowDemoData ? demoListings : []
  try {
    const data = await graphListings(env)
    return { data: data ?? fallback, source: data ? "graph" as const : allowDemoData ? "demo" as const : "unavailable" as const }
  } catch (error) {
    console.error("Graph listing query failed", error)
    return { data: fallback, source: allowDemoData ? "demo" as const : "unavailable" as const }
  }
}
...
app.get("/v1/authors/:address", async (c) => {
  ...
  const allowDemoData = c.env.APP_ENV !== "production"
  const emptyDashboard = { totalSales: 0, grossRevenueUsdc: "0.00", sales: [], skills: [] }
  const demoDashboard = { totalSales: 2964, grossRevenueUsdc: "741.00", sales: [] }
  if (!c.env.GRAPH_API_URL) return c.json({ data: allowDemoData ? demoDashboard : emptyDashboard, source: allowDemoData ? "demo" : "unavailable" })
  try {
    const response = await fetch(c.env.GRAPH_API_URL, { method: "POST", headers: { "content-type": "application/json", ...(c.env.GRAPH_API_KEY ? { authorization: `Bearer ${c.env.GRAPH_API_KEY}` } : {}) }, body: JSON.stringify({ query: "query AuthorDashboard($author: Bytes!) { author(id: $author) { totalSales grossRevenue skills { id metadataURI price majorVersion totalSales active } } purchases(first: 10, orderBy: timestamp, orderDirection: desc, where: { skill_: { author: $author } }) { id buyer amount timestamp transactionHash skill { metadataURI id } } }", variables: { author: address } }) })
    if (!response.ok) throw new Error(`The Graph query failed (${response.status})`)
    ...
    if (!author) return c.json({ data: { totalSales: 0, grossRevenueUsdc: "0.00", sales: [], skills: [] }, source: "graph" })
    const grossRevenue = Number(author.grossRevenue)
    return c.json({ data: { totalSales: author.totalSales, grossRevenueUsdc: usdc(grossRevenue), sales: (payload.data?.purchases ?? []).map((purchase) => ({ id: purchase.id, skill: purchase.skill.metadataURI.replace("skillsbay://", "") || purchase.skill.id, buyer: `${purchase.buyer.slice(0, 6)}…${purchase.buyer.slice(-4)}`, amount: usdc(purchase.amount), occurredAt: new Date(Number(purchase.timestamp) * 1_000).toLocaleString(), transactionHash: purchase.transactionHash })), skills: author.skills.map((skill) => { const sourceId = skill.metadataURI.replace("skillsbay://", "") || skill.id; const parsed = splitSkillId(sourceId); return { id: skill.id, namespace: parsed?.namespace ?? "onchain", slug: parsed?.slug ?? skill.id.slice(2, 10), title: parsed?.slug?.split("-").map((word) => word[0].toUpperCase() + word.slice(1)).join(" ") ?? "Onchain skill", priceUsdc: usdc(skill.price), version: `${skill.majorVersion}.0.0`, paidInstalls: skill.totalSales, active: skill.active } }) }, source: "graph" })
  } catch (error) {
    console.error("Graph author query failed", error)
    return c.json({ data: allowDemoData ? demoDashboard : emptyDashboard, source: allowDemoData ? "demo" : "unavailable" })
  }
})
