...
import { authorSales, skills as demoSkills } from "@/data/mock-marketplace"
...
export function AuthorOverview() {
  ...
  const isDemoData = dashboard.data?.source === "demo"
  const data = dashboard.data?.data
  const useDemoData = isDemoData
  const sales = useDemoData ? authorSales : data?.sales ?? []
  const publishedSkills = useDemoData ? demoSkills.slice(0, 3).map((skill) => ({ ...skill, active: true })) : data?.skills ?? []
  const totalSales = useDemoData ? 2964 : data?.totalSales ?? 0
  const grossRevenue = useDemoData ? "741.00" : data?.grossRevenueUsdc ?? "0.00"
  const walletLabel = author.walletAddress ? `${author.walletAddress.slice(0, 6)}…${author.walletAddress.slice(-4)}` : "0x8dF2…7A31"

  return <div className="grid gap-6">
    <div className="flex items-center gap-1.5 text-xs text-muted-foreground"><StatusDot className={isLiveGraphData ? undefined : "bg-amber-500"} /> {isLiveGraphData ? "Live Graph index" : isDemoData ? "Local demo data" : "Graph index pending — connect the deployed Graph endpoint for account activity."}</div>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <MetricCard label="Published skills" value={publishedSkills.length.toString()} detail={isLiveGraphData ? "Registered on Base Sepolia" : isDemoData ? "Demo marketplace catalogue" : "Awaiting Graph index"} icon={<BarChart3 className="size-4" />} />
      <MetricCard label="Paid installs" value={totalSales.toLocaleString()} detail={isLiveGraphData ? "Indexed purchase receipts" : isDemoData ? "Demo receipt history" : "Awaiting indexed receipts"} icon={<Download className="size-4" />} />
